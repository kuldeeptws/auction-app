					...
					<tr>
						...
						<td>
							<select name="category">
								<?php $_cats1_count = (isset($this->_tpldata['cats1'])) ? sizeof($this->_tpldata['cats1']) : 0;if ($_cats1_count) {for ($_cats1_i = 0; $_cats1_i < $_cats1_count; ++$_cats1_i){$_cats1_val = &$this->_tpldata['cats1'][$_cats1_i]; ?>
									<option value="<?php echo $_cats1_val['CAT_ID']; ?>"<?php if ($_cats1_val['SELECTED']) {  ?> selected="true"<?php } ?>><?php echo $_cats1_val['CAT_NAME']; ?></option>
								<?php }} ?>
							</select>
						</td>
					</tr>
					<tr>
						...
						<td>
							<select name="secondcat">
								<?php $_cats2_count = (isset($this->_tpldata['cats2'])) ? sizeof($this->_tpldata['cats2']) : 0;if ($_cats2_count) {for ($_cats2_i = 0; $_cats2_i < $_cats2_count; ++$_cats2_i){$_cats2_val = &$this->_tpldata['cats2'][$_cats2_i]; ?>
									<option value="<?php echo $_cats2_val['CAT_ID']; ?>"<?php if ($_cats2_val['SELECTED']) {  ?> selected="true"<?php } ?>><?php echo $_cats2_val['CAT_NAME']; ?></option>
								<?php }} ?>
							</select>
						</td>
					</tr>
					...
					<tr>
						...
						<td>
							<select name="duration">
								...
								<option value="<?php echo $_dur_val['DAYS']; ?>"<?php if ($_dur_val['SELECTED']) {  ?> selected<?php } ?>><?php echo $_dur_val['DESC']; ?></option>
								<?php }} ?>
							</select>
						</td>
					</tr>
...
<?php $_gallery_count = (isset($this->_tpldata['gallery'])) ? sizeof($this->_tpldata['gallery']) : 0;if ($_gallery_count) {for ($_gallery_i = 0; $_gallery_i < $_gallery_count; ++$_gallery_i){$_gallery_val = &$this->_tpldata['gallery'][$_gallery_i]; ?>
							<div style="width:50px; float: left;">
								<a href="<?php echo (isset($this->_rootref['SITEURL'])) ? $this->_rootref['SITEURL'] : ''; ?>/<?php echo $_gallery_val['V']; ?>" title="<?php echo $_gallery_val['V']; ?>" target="_blank">
									<img src="<?php echo (isset($this->_rootref['SITEURL'])) ? $this->_rootref['SITEURL'] : ''; ?>getthumb.php?fromfile=<?php echo $_gallery_val['V']; ?>&auction_id=<?php echo (isset($this->_rootref['ID'])) ? $this->_rootref['ID'] : ''; ?>" border="0" hspace="10">
								</a>
...
<?php }} ?>
					...
					<tr>
						...
						<td>
							<input type="radio" name="buy_now_only" value="0" <?php echo (isset($this->_rootref['BN_ONLY_N'])) ? $this->_rootref['BN_ONLY_N'] : ''; ?>> <?php echo ((isset($this->_rootref['L_no'])) ? $this->_rootref['L_no'] : ((isset($MSG['no'])) ? $MSG['no'] : '{ L_no }')); ?>
							<input type="radio" name="buy_now_only" value="1" <?php echo (isset($this->_rootref['BN_ONLY_Y'])) ? $this->_rootref['BN_ONLY_Y'] : ''; ?>> <?php echo ((isset($this->_rootref['L_yes'])) ? $this->_rootref['L_yes'] : ((isset($MSG['yes'])) ? $MSG['yes'] : '{ L_yes }')); ?>
						</td>
					</tr>
					<tr>
