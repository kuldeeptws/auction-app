...
//				throw new ErrorException(SiteModule::__('Form sending from preview is not available'));
			...
			foreach ($fields as $idx => $field) {
				...
				if ($field["type"] == "textarea")
					...
					if( is_array($value) ) {
						$message .= "<tr><td colspan=\"2\"><strong>$name: </strong></td></tr>\n<tr><td colspan=\"2\">" . nl2br(implode(PHP_EOL, $value)) . "</td></tr>\n";
					} else {
						$message .= "<tr><td><strong>$name: </strong></td><td>" . ($value ? tr_('Yes') : tr_('No')) . "</td></tr>\n";
					}
				}
				else if ($field["type"] == "range") {
					$message .= "<tr><td><strong>$name: </strong></td><td>" . nl2br($value["from"]) . ' - ' . nl2br($value["to"]) . "</td></tr>\n";
				}
				else
...
function checkSiteRedirects(SiteInfo $siteInfo, SiteRequestInfo $requestInfo, array $redirectItems) {
	...
	if (!$requestUri || !count($redirectItems)) {
		return;
	}
	...
	foreach ($redirectItems as $item) {
		...
		if (($item['exact'] && trim($item['fromUrl'], '/') == $requestUri) || (!$item['exact'] && strpos($requestUri . '/', trim($item['fromUrl'], '/') . '/' ) === 0)) {
			if ($item['toUrl']['type'] == 'page') {
				$defLang = ($siteInfo->defLang ? $siteInfo->defLang : null);
				if ($item['exact']) {
					$redirectTo = getBaseUrl() . getPageUri($item['toUrl']['url'], $defLang, $siteInfo) . (isset($item['toUrl']['anchor']) ? '#' . $item['toUrl']['anchor'] : '');
				}
				else {
					$toUrl = getPageUri($item['toUrl']['url'], $defLang, $siteInfo);
					$redirectTo = getBaseUrl() . str_replace(trim($item['fromUrl'], '/') . '/', $toUrl, $requestUri . '/') . (isset($item['toUrl']['anchor']) ? '#' . $item['toUrl']['anchor'] : '');
				}
			}
			elseif ($item['toUrl']['type'] == 'url') {
				$redirectTo = $item['toUrl']['url'];
			}
			elseif ($item['toUrl']['type'] == 'file') {
				$scheme = parse_url($item['toUrl']['url'], PHP_URL_SCHEME);
				$redirectTo = $scheme ? $item['toUrl']['url'] : getBaseUrl() . $item['toUrl']['url'];
			}
			break;
		}
	}
